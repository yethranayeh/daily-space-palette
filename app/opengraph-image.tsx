...
export const alt = `${SITE_NAME} — Today's color palette from NASA's APOD`;
export const size = { width: 1200, height: 630 };
export const contentType = "image/png";

async function fetchApodImage(): Promise<{
  dataUrl: string;
  buffer: Buffer;
  title: string;
  date: string;
  ...
  try {
    const res = await fetch(
      `${APOD_URL}?api_key=${process.env.NASA_API}&date=${getFormattedDate()}`,
    );
    ...
    return { dataUrl, buffer, title: json.title ?? "", date: json.date ?? "" };
  } catch {
    return null;
  }
}
...
export default async function Image() {
  const apod = await fetchApodImage();
